...
def evaluate_records(records, candle_rows, *, as_of_ms, target_pct=2., cost_bps=12., latency_ms=0):
    ...
    for record in records:
        ...
        if strategy not in HORIZONS or not isinstance(symbol, str) or not re.fullmatch(r"\w+USDT", symbol):
        ...
        if record.get("performance_market") != market or record.get("direction") != "LONG":
            ...
            continue
        if record.get("horizon_hours") != HORIZONS[strategy]:
        ...
        event_id = (supplied_id if isinstance(supplied_id, str)
                    and re.fullmatch(r"[0-9a-f]{32}", supplied_id)
        ...
        identity = (strategy, symbol, market, contract, record.get("universe"),
                    record.get("config_version"), record.get("engine_config_hash"), bar_ms)
        if event_id in conflicts:
            rejected["conflicting_event_provenance"] += 1
            continue
        if event_id in chosen:
            previous = chosen[event_id]
            if previous["identity"] != identity:
                del chosen[event_id]
                conflicts.add(event_id)
                rejected["conflicting_event_provenance"] += 2
                continue
            rejected["duplicate_delivery"] += 1
            if previous["confirmed"] and (not confirmed or previous["delivered"] <= delivered):
                continue
            if not previous["confirmed"] and not confirmed:
                continue
        chosen[event_id] = dict(identity=identity, confirmed=confirmed, delivered=delivered)
    measured = []
    for event_id, item in sorted(chosen.items()):
        strategy, symbol, market, contract, universe, config, engine, _bar_ms = item["identity"]
        ...
        measured.append({"event_id": event_id, "symbol": symbol, "strategy": strategy,
                         ...
                         "evidence_source": ("forward_confirmed_delivery" if item["confirmed"]
...
def main():
    ...
    if args.output.resolve() in (args.signals_log.resolve(), args.candles_jsonl.resolve()):
    ...
    report = evaluate_records(signals, candles, as_of_ms=utc_ms(args.as_of),
